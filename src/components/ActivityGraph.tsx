...
export function ActivityGraph({ daily }: Props) {
  ...
  const availableYears = useMemo(() => {
    ...
    for (const d of daily) years.add(parseInt(d.date.slice(0, 4)));
    ...
    <div ref={containerRef} style={{
        ...
        <div style={{ overflowX: "auto", paddingBottom: 4 }}>
          <div style={{ display: "flex", marginLeft: labelW, marginBottom: 2 }}>
            {Array.from({ length: totalWeeks }).map((_, col) => {
              ...
              return (
                <div key={col} style={{
                  width: cellSize + cellGap, fontSize: 10,
          ...
          <div style={{ display: "flex" }}>
            <div style={{
              ...
              marginRight: 2, width: labelW - 2,
            }}>
              {DAY_LABELS.map((label, i) => (
                <div key={i} style={{
                  height: cellSize, fontSize: 10, color: "var(--text-secondary)",
                  fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "flex-end",
                }}>
                  {label}
                </div>
              ))}
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          background: HEAT_COLORS[level], cursor: "pointer",
                          ...(isToday ? { outline: "2px solid var(--accent-purple)", outlineOffset: -1 } : {}),
                        }}
                        onMouseEnter={(e) => {
      ...
      <div style={{
        ...
        {HEAT_COLORS.map((color, i) => (
          <div key={i} style={{ width: 8, height: 8, borderRadius: 1, background: color }} />
        ))}
      ...
      {tooltip && (
        <div style={{
          position: "absolute",
          left: tooltip.x, top: tooltip.y - 4,
          ...
          <div>{formatTokens(tooltip.tokens)} tokens &middot; {formatCost(tooltip.cost)}</div>
        </div>
      )}
